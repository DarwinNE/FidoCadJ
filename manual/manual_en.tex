...
\title{\Huge\color{webbrown} FidoCadJ 0.23.1 \\ the user manual} 
...
Roby IZ1CYN for the useful discussions about librairies and to have written 
...
The appendix~\ref{specifics} briefly describes how to install FidoCadJ under 
...
place a ceramic capacitor in parallel with the power supply. \graffito{Be carefult with the
...
Starting from FidoCadJ version 0.23, the software allows to draw automatically arrows heads\index{arrows} at the beginning and at the end of lines and Bézier curves.
...
\end{figure}
The possibility of chosing a dashing style and putting arrowheads was not comprised in the original FidoCad format. This unfortunately means that FidoCadJ drawings using those functionalities are not completely backward compatible with FidoCad\index{FidoCad} per Windows.
...
From version 0.23, thanks to Roby IZ1CYN, I could include the IHRaM 2.1 library\index{IHRaM library} directly inside the FidoCadJ distribution. I have done this because among all the libraries I have seen, this one has appeared to be one of the most complete and rationally constructed.
...
of the version embedded in the program.

Other files with \lstinline!.fcl! extension in the library search path will be considered as libraries and FidoCadJ will try to load them when it is starting. This is done only once, so if you need to add or remove libraries in the directory you use for them, remember to restart FidoCadJ.
...
Since I added a few extensions which are only available on FidoCadJ, I will try to describe them here as well.
...
FCJ e 
\end{lstlisting}
where $e$ is an integer giving the dashing style.
...
FidoCadJ allows to activate a ``strict FidoCad compatibility mode'', in which all extensions are disabled. This way, FidoCadJ will be perfectly compatible with the original FidoCad\index{FidoCad}.
...
\caption{Figura~\ref{fig_gyrator} as it would appear under FidoCad for Windows.}
\label{fig_gyrator_n}
\end{figure}
...
\myChapter{Platform-specific information} \label{specifics} 
...
\subsection{How to download and execute FidoCadJ under MacOSX}
FidoCadJ can work with a MacOSX\index{MacOSX} version more recent than 10.3.9 (Panther\index{Panther}).
The reason is that under the hood FidoCadJ needs at least the 1.4 version of Java, which Apple provides with this operating system.

Even if you can use directly the Java archive \lstinline!fidocad.jar! as you would do under other operating systems, under MacOSX you can use the specifically tailored application. Everything works just like a native application: you can download the disk image at the following link:
