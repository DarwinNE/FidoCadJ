...
section~\ref{installazione_linux} of this manual, dealing with its installation on Linux\index{Linux}. Thanks to Max2433BO for advices about Linux and for his patient work on GitHub issues.
...
What matters is impressions of who exploits the program, much
...
on Oracle\index{Oracle}), as well as modern incarnations of macOS, but the philosophy remains the same. We will describe the features of the program, as well as its basic elements (the primitives)\index{primitive}, which compose a FidoCadJ drawing. %
\begin{figure}
...
We can then continue placing the other parts of the circuit, a resistor\index{resistor} (Standard Library/Discrete devices/Resistor)
...
is installed on the machine. Using Oracle's\index{Oracle} terminology,
...
represented. The command used to call a macro is \lstinline!MC!\index{MC},
...
FidoCAD\index{FidoCAD}. These are the standard library\index{standard library}
...
Under the hood, FidoCadJ needs at least version 9 of Java. For marketing reasons, Apple does not install Java with the last versions of macOS\index{macOS}. By the way, Apple does not allow to distribute his App Store\index{App Store} software based on Java or distributed under GPL. This is one of the most important reasons for which it is quite improbable that FidoCadJ will be ported to iPad\index{iPad} and iPhone\index{iPhone}.
...
Prerequisite: JRE\index{JRE} 9 from Oracle\index{Oracle} and/or OpenJDK\index{OpenJDK}
9 JRE\index{JRE} (or successive versions compatible with the program's
...
\begin{lstlisting}
# mv /home/<user>/fidocadj-0.24.8.jar /usr/local/bin/fidocadj
\end{lstlisting}
...
\label{inst_grafica}
In the example, it will be an Ubuntu\index{Ubuntu} 8.04, but things do not change much for older or newer versions:
...
\item {We can select the tab ``Open as'' and select ``OpenJDK Java 9 Runtime'' or ``Oracle Java 9 Runtime'' (or a successive version).}%, as it can be seen in figure~\ref{fig_java6}.}

\item {Let us click on ``Close'' and we are now ready to execute FidoCadJ: a double click on the executable, or we can add it to the main menu. The command to add is just \lstinline!/usr/local/bin/fidocadj/fidocadj-0.24.8.jar!.}
\end{itemize}
