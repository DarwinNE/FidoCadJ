...
\title{\Huge\color{webbrown} FidoCadJ 0.23.3 \\ the user manual} 
...
In April 2010, FidoCadJ has been integrated in the Italian web site \href{www.electroportal.it}{www.electroportal.it}. It silently runs on the server and it is used to automagically convert drawings posted by users of the forum.
I would like to express my gratitude to the admin and webmaster and to all other users of this site as they gave me a lot of very useful ideas about batch controlling FidoCadJ.
...
place a ceramic capacitor in parallel with the power supply. \graffito{Be careful with the
...
The most simple option is \lstinline!-n!, with which the software\dots does nothing, i.e. it does not activate the GUI and just exits. Obviously, this option is not so much useful alone, but it will be precious when using in combination with other functionalities we will describe in a minute.
...
We can thus have a look at the example given in the help:
	 	
\begin{lstlisting}
java -jar fidocadj.jar -n -c 800 600 png out.png test.fcd
\end{lstlisting}

FidoCadJ will run without activating the GUI and will export in the png\index{PNG} format the file \lstinline!test.fcd!. The output file will be called \lstinline!out.png! and will have a 800x600 pixel size.
...
To know the total size (in logical units) of a schematic, you can use the \lstinline!-s! option. Remember anyway that, during the exports, FidoCadJ adds always a 20 pixel margin to the dimensions extracted from the circuit.
...
\caption{Figure~\ref{fig_gyrator} as it would appear under FidoCad for Windows.}
\label{fig_gyrator_n}
\end{figure}
