...
\begin{figure}
\includegraphics[width=0.95 \textwidth]{fidocadj_macosx.png}
...
%
\begin{figure}
\includegraphics[width=0.95\textwidth]{fidocadj_metal.png}

\caption{An old preliminary version of FidoCadJ, with the Look and Feel Metal\index{Metal}.}
...
You will notice that once a button in the first line is depressed, it remains in that position until another function from the toolbar is selected.\marginpar{This
...
drawing primitive\index{primitive} will be used\footnote{For more information on the drawing elements, see~\ref{sec_primitive}.%
...
double-click to terminate the insertion
\tabularnewline
...
\textsc{Polyline}\index{polyline}  & Draw a polyline filled or empty. Double-click or press \keyevidence{Esc},
...
\textsc{PCB track}\index{PCB track}  & Draw a PCB track. The default width can be modified through the dialog
...
``File/New''. We then start by placing in the drawing area the two transistors\index{transistor} around which our schematic
is built. To do so, we need the symbols (aka ``macros'') contained in the standard library\index{standard library}, which is loaded by default and is
...
\caption{We are too close to the top edge of the sheet: let us select the whole
...
we are writing and the code is added automatically\index{code}.
...
The layers' ordering is important, as layer with a lower number
are drawn earlier: elements on successive layers cover the
ones already drawn.
...
(and a lot of erasers) result in time saved by obtaining a clear
...
the publicity flyers from major CAD companies, yet is no doubt that
these are still tasks where the user's experience still plays an
...
designed for a polyester capacitor.  It probably has the right dimensions. Let us not forget that FidoCadJ is thought as a replacement for the transfers\index{R41 transfers}\dots
...
probably be a track so thin that will detach from the board during soldering.}
Let us put the writing on the top side as well. To write on the copper layer,
after selecting the proper layer it is also necessary to mirror
...
we want to modify when we are in selection mode. You may need to experiment a bit, to obtain the right size for the characters. To give an
...
Once the drawing is completed, we probably need to print\index{printing}
...
FidoCadJ allows to draw arrow heads\index{arrows} at the beginning and at the end of lines, Bézier curves and natural cubic splines. It also allows to specify whether the arrow should be at the beginning or at the end of an element (or both), and offers you a few different arrow styles. Arrow length can be negative: in this case, the arrow extends outside the line or curve. Try this and you will see the result.

A few stroke styles\index{stroke styles} are also available, for technical or mechanical drawings. Figure~\ref{fig_gyrator} shows an example in which an electrical circuit (a GIC\index{GIG}) is enclosed in a dashed rectangle. An arrow at the end of a Bézier curve is also being used. By doing double-click on this element, FidoCadJ shows a parameter window that should be similar to the one shown in figure~\ref{fig_parametri_bezier}. You can notice that the option box ``Arrow at start'' has been checked. FidoCadJ thus traces an arrowhead by taking care of his orientation.
There are several different drawing styles for the arrow\index{arrow styles} as well for dashing \index{dashing styles}. You may try to play a little bit with them to appreciate their differences. If the arrow length is negative, the arrow extends outside the length of the element, pointing towards it.
...
in FidoCadJ) specifies whether it is vector or bitmap\footnote{The code structure of FidoCadJ allows the addition of other file format
...
are not be translated. This allows the introduction of \LaTeX{}\index{\LaTeX{}@\LaTeX}
...
tracks are not available yet. These are not exported to the Eagle
...
is present, its content supersedes the standard library\index{library!standard library} directly available in the application. Analogously, if a file named
...
\end{figure}
\item Choose ``Symbol-o-matic'' and a dialog for the definition of the details of the newly created symbol appears (see figure~\ref{fig_new_symbols}).
\begin{figure}
  \begin{center}
...
Since I added a few extensions that are only available on FidoCadJ, I describe them here as well. Remember that since version 0.23.4, FidoCadJ uses only the UTF-8\index{UTF-8} encoding\index{encoding} on all platforms.
...
\end{itemize}
We will describe each of them. In general, every element\index{element}
...
exactly as it was advanced text and it uses the command \lstinline!TY!\index{TY}
...
FidoCadJ treats this object as it was placed on layer zero (circuit\index{circuit}).
...
the horizontal one, and that aspect ratio is chanced only if
...
FidoCadJ allows to activate a ``strict FidoCAD compatibility mode'', in which all extensions are disabled. This way, FidoCadJ is perfectly compatible with the original FidoCAD\index{FidoCAD}, except that FidoCadJ continues using the UTF-8\index{UTF-8} encoding\index{encoding} instead of the old CP-1252\index{CP-1252} used by FidoCAD\index{FidoCAD}.
FidoCAD for Windows is unable to understand the additional informations carried by the \lstinline!FCJ!\index{FCJ} command. When reading with the original FidoCAD a FidoCadJ drawing containing some extension, the program prompts a few errors and gives a result that is different from the original FidoCadJ drawing.
...
Let us create a new directory (but at first we will become a superuser\index{superuser} by using \lstinline!su! or \lstinline!sudo -s!):
...
\begin{lstlisting}
# mv /home/<user>/fidocadj-0.24.8.jar /usr/bin/fidocadj
\end{lstlisting}

Let us make the file an executable

\begin{lstlisting}
# chmod +x /usr/bin/fidocadj/fidocadj-0.24.8.jar
\end{lstlisting}
...
\begin{lstlisting}
$ /usr/bin/fidocadj/fidocadj-0.24.8.jar
\end{lstlisting}
...
\begin{itemize}
\item {Let us download the file from the browser, or with Gwget or similar tools.}
...
\item {Let us do right click on the file, from the window we select the tab ``rights'', we add the check mark in front of the ``Allow the execution of the file as a program'', as shown in the figure~\ref{fig_permessi}}
...
\item {Let us click on ``Close'' and we are now ready to execute FidoCadJ: a double click on the executable, or we can add it to the main menu. The command to add is just \lstinline!/usr/bin/fidocadj/fidocadj-0.24.8.jar!.}
\end{itemize}
