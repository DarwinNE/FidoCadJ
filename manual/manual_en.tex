...
\chapter*{\pdfbookmark[1]{Abstract}{Abstract}Abstract}
...
\chapter*{\pdfbookmark[1]{Aknowledgements}{Aknowledgements}Acknowledgements}
...
\chapter*{\pdfbookmark[1]{License FidoCadJ}{License FidoCadJ}
FidoCadJ License}
...
I wrote the core of the applet about in March 2007. A few months later,
...
and energy on something that is not completely portable does not
...
different when running on other operating systems, as well as modern incarnations of macOS, but the philosophy remains the same. We will describe the features of the program, as well as its basic elements (the primitives)\index{primitive}, which compose a FidoCadJ drawing. %
\begin{figure}
...
%
\begin{figure}
\includegraphics[width=0.95\textwidth]{menu_tool_drawing_bars.png}
\caption{The top portion of the FidoCadJ window, showing the menu, tool and drawing bars.}
\label{fig_menu_tool_drawing_bars}
\end{figure}

In the toolbar\index{toolbar} at the top of the window (as shown in figure \ref{fig_menu_tool_drawing_bars}), you can
...
functions, the commands and describes the corresponding actions.
You will notice that once you depress a button in the first line, it remains in that position until you select another function from the toolbar.\marginpar{This
...
application, since it is arguably not something that we may want to change
every day. Figures \ref{fig_fidocadj} and \ref{fig_menu_tool_drawing_bars} show the
...
\textsc{B\'ezier}\index{B\'ezier}  & Draw a B\'ezier curve\tabularnewline
...
\item B\'ezier curve \index{B\'ezier}
...
\begin{table}
\centering \begin{tabular}{ccp{0.7\textwidth}}
...
A B\'ezier\index{B\'ezier} curve segment, in its cubic variant, is identified
...
four control points of the B\'ezier\index{B\'ezier} curve segment, while
$l$ is the layer, identified by a whole number between 0 and 15.
...
where $aaaa$ is a double precision floating point value (positive) that gives the diameter of the electrical connection, in FidoCadJ logical units.
 ...
 The stroke width for the ``pen'' used during the drawing of electrical schematics can be modified. FidoCadJ adopts the command \lstinline!FJC A!\index{FJC A} to specify the stroke width of the lines, ovals, B\'ezier, splines, rectangles and polygons. The width can be a double precision constant.
\begin{lstlisting}
FJC A aaaa
\end{lstlisting}
...
FidoCadJ is designed to tolerate errors\index{error tolerance} and 
