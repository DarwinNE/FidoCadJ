...
\title{\Huge\color{webbrown} FidoCadJ 0.24.2 \\ user manual} 
...
as printed circuit boards\index{PCB}. It is widespread in particular in the italian
...
To activate this behavior, you have to activate ``Split non standard macros when saving'' in the ``FidoCadJ extensions'' tab of the ``Preferences'' window. During copy/pasting, a different shortcut is available in the ``Edit'' menu, called ``Copy, split non standard macros'', in order to make sort that the copied drawing will have all the non standard macros split.
...
A few menu actions are available on symbols in the library tree and the drag and drop\index{Drag and drop} is also supported there. Once you select a symbol in an user library and do a right-click on it, a popup menu appears, as shown in figure~\ref{fig_popup_menu_lib}. Here you have several options:
\begin{figure}
  \begin{center}
