...
\title{\Huge\color{webbrown} FidoCadJ 0.22 \\ le manuel de l'utilisateur} 
...
Je remercie F. Bertolazzi pour m'avoir motivé à faire en sorte que le logiciel soit utilisable et qui a assemblé avec patience la librairie CadSoft Eagle permettant l'exportation vers ce logiciel. Je remercie Celsius, qui a testé le fonctionnement du logiciel pour la réalisation de circuits imprimés et les librairies, ainsi qu'Andrea D'Amore, pour ses conseils sur comment améliorer l'apparence du logiciel sous Apple Macintosh, à partir de la version 0.21.1.
...
La macro qui nous intéresse est appelé <<NPN sans écran>> et se trouve dans la catégorie <<Diodes et transistors>>, à l'intérieur de la <<Librairie standard>>. En faisant click pour sélectionner la macro\index{macro} désirée, il est ensuite possible de l'insérer à la position voulue dans le dessin.
...
Faites attention aussi à travailler sur la couche\index{couche} correcte, c'est à dire la 1: c'est à dire le cuivre du côté des soudures.\graffito{Attention à la largeur des pistes: ce qui a l'air d'être une autoroute sur l'écran, se révélera dans la réalité une fine ligne prête à se décoller pendant la soudure.}
L'utilisation d'un polygone pour les plans d'alimentation peut être utile pour disposer de connexions à basse impédance parasite. Avec un peu d'attention, normalement vous devriez avoir le résultat montré en figure~\ref{fig_amplificateur_phase2}.
...
\textsc{pdf}\index{PDF}
&
Le format Portable Document Format, de Adobe.
\\
\bottomrule
\end{tabular}
...
Une autre possibilité intéressante, même si à la rigueur il s'agit plutôt d'une caractéristique de Java\index{Java} plus que de FidoCadJ lui même, est qu'il est possible de modifier l'apparence du logiciel en changent ce qui en jargon Java l'on appelle le <<look and feel>>\index{look \& feel}). Ceci peut être utile par exemple pour intégrer plus le logiciel à l'intérieur d'un environnement Windows\index{Windows}. Dans ce cas, la ligne de commande à utiliser est la suivante:\graffito{Il faut reconnaître que parfois Java n'est pas très synthétique.}
\begin{lstlisting}
...
LI x1 y1 x2 y2 l
\end{lstlisting}
Le point $(x_1, y_1)$ représente les coordonnées initiales, $(x_2, y_2)$ celles finales et $l$ est le numéro de la couche, comprise entre 0 et 15.
...
RV x1 y1 x2 y2 l
\end{lstlisting}
les coordinates $(x_1, y_1)$ représentent le premier point de la diagonale, $(x_2, y_2)$ le deuxième point et $l$ est le numéro de la couche, compris entre 0 et 15.
...
\end{lstlisting}
le point $(x_1, y_1)$ spécifie où les caractères <<texte à écrire>> seront positionnés. Nous pouvons remarquer que l'indication de la couche n'apparaît pas. FidoCadJ traitera cet objet comme appartenant à la couche zéro (circuit\index{circuit}).
...
La couche utilisée est représentée par le terme $l$, tandis que  $f$ indique la police à utiliser, ou il est une étoile\index{étoile}, pour indiquer l'utilisation de la police standard, qui est Courier\index{Courier}, dont on peut voir un exemple en figure~\ref{fig_courier_sample}.
Si le nom de la police contient des espaces, ceux derniers sont remplacés par le symbole +\index{+}.

Actuellement, la seule police de caractères utilisée et supportée par FidoCadJ est Courier\index{Courier}, police à espacement fixe et avec empattements, qui est aussi la police utilisée par défaut par FidoCad\index{FidoCad}.
...
PV x1 y1 x2 y2 ... l
\end{lstlisting}
les points $(x_1, y_1)$, $(x_2, y_2)$\dots sont les coordonnées des sommets qui définissent le polygone et $l$ est le numéro de la couche, indiquée avec une chiffre comprise entre 0 et 15.
...
SA x1 y1 l
\end{lstlisting}
Avec FidoCadJ, le diamètre du cercle est fixé à 2 unités logique et ne peut pas être modifié par l'utilisateur.
...
\label{FCJ_extension}
\`A partir de la version 0.21, FidoCadJ a introduit des extensions\index{estensions FidoCadJ} au format FidoCad originel. Ces extensions sont identifiées par la commande \lstinline!FCJ!\index{FCJ}. 
Pour l'instant, la seule extension implementée est celle qui permet d'associer un nom et une valeur à chaque macro. L'utilisation est la suivante:
\begin{lstlisting} 
[FIDOCAD]
MC 40 30 0 0 080
FCJ
...
\end{lstlisting}
La presence de la commande \lstinline!FCJ! indique que la définition de la macro n'est pas terminée, mais que les champs qui indiquent le nom et la valeur du composant sont spécifiés juste après, par les deux lignes qui contiennent les commandes \lstinline!TY!. Actuellement, dans cette façon de fonctionner, le logiciel se sert seulement des coordonnées des points où écrire le texte.

Cette façon de procéder a l'avantage que si un fichier contient une extension \lstinline!FCJ! et il est lu par FidoCad\index{FidoCad}, ce logiciel produira un message d'erreur, mais le dessin obtenu sera parfaitement identique à celui de FidoCadJ.

FidoCadJ peut être configuré pour ne pas émettre la commande \lstinline!FCJ!\index{FCJ} pendant la sauvegarde d'un fichier, ou pendant les opérations de copier/coller. De cette façon, nous obtiendrons un résultat parfaitement compatible avec FidoCad\index{FidoCad}, mais en perdant l'association logique entre la macro, son nom et sa valeur.
...
Pour cette raison, certains détails sur le fonctionnement du logiciel sont légèrement différents quand FidoCadJ se rend compte d'être en train de tourner sur une plate-forme Apple:
\begin{itemize}
...
\item Les entrées <<Préférences...>> et <<\`A propos de FidoCadJ>> se trouvent à leur place, c'est à dire à l'intérieur du menu <<FidoCadJ>>.
...
\mbox{ }
\vfill
Ce manuel a été redigé en utilisant \textsc{pdf}\LaTeX\ sous MacOSX, en adoptant la classe \href{http://www.ctan.org/tex-archive/macros/latex/contrib/classicthesis/}{classicthesis}.
