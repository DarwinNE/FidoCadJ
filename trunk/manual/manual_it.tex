...
\title{\Huge\color{webbrown} FidoCadJ 0.24 \\ il manuale dell'utente} 
...
Una spline cubica\index{spline} viene creata a partire da un certo numero di vertici. La curva passa in tutti i vertici e viene calcolata in modo che il suo percorso sia molto liscio. All'interno di FidoCadJ, una spline cubica è identificata dai comandi \lstinline!CV!\index{CV} e \lstinline!CP!\index{CP}:
\begin{verbatim}
CV aa x1 y1 x2 y2 ... l
CP aa x1 y1 x2 y2 ... l
\end{verbatim}
...
La lunghezza della linea può quindi variare a seconda del numero di vertici presenti.
Il numero massimo di vertici\index{numero massimo di vertici} disponibili (come con i poligoni) è fissato arbitrariamente a 20, per evitare di avere a che fare con linee troppo lunghe. Questa primitiva è stata introdotta da FidoCadJ a partire dalla versione 0.24 e non è disponibile in FidoCAD\index{FidoCAD}.

\textbf{Estensione FidoCadJ}: i comandi \lstinline!CV!\index{CP} e \lstinline!CV!\index{CV} possono essere seguiti nella riga successiva da un'estensione:
\begin{verbatim} 
FCJ e nv
\end{verbatim}
dove $e$ è un intero che fornisce lo stile del tratteggio. Se $nv$ è uguale a 1, la linea \lstinline!FCJ! dev'essere seguita da due comandi \lstinline!TY! che specificano rispettivamente il nome ed il valore associati a quest'elemento.
