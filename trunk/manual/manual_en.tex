...
\hypersetup{%
    ...
    pdfkeywords={FidoCad, FidoCadJ, CAD, electronic schematic, printed circuit board},%
    pdfcreator={pdfLaTeX},%
...
\title{\Huge\color{webbrown} FidoCadJ 0.21 \\ user manual} 
...
This work is covered by the Creative Commons Public License version 2.5
or more recent. The entire texte of this licence is available at the address\\
...
This document is the FidoCadJ official user manual. After a short introduction of the history and the born of this software, we will describe the basic use of FidoCadJ. Our goal will be to learn how to write a very simple electronic schematics as well as a printed circuit board. This manual will end with a detailed description of the FidoCad (and thus FidoCadJ) format, which has not yet been documented.
\endgroup			
...
A lot of people used this software from its first versions and they helped me by providing their advices. I thus want to thank the it.hobby.elettronica\index{it.hobby.elettronica} newsgroup\index{newsgroup} partecipants for their very fruitful discussions.
...
I would like to thank F. Bertolazzi who has given very useful advices about the usability of this software. He also assembled the CadSoft Eagle\index{Eagle} compatibility library, useful to export FidoCadJ drawings under Eagle.
Many thanks to Celsius, who tested the software functionalities for PCB realization, as well as its libraries. Thanks to Andrea D'Amore, for his advices concerning FidoCadJ visual aspect on the Apple Machintosh from the 0.21.1 version.

\chapter*{FidoCadJ Licence}
\pdfbookmark[1]{Licence FidoCadJ}{Licence FidoCadJ}

 
Copyright \copyright\ 2007-2009  Davide Bucci \href{mailto:davbucci@tiscali.it}{davbucci@tiscali.it}
...
In this chapter, we will briefly introduce FidoCadJ. In particular, we will give a description of the phylosophy behind this software, as well as a brief history of its development.

\section{FidoCadJ's philosophy}

FidoCad\index{FidoCad} (without J at the end) is a vectorial drawing software particularly suit for electrical schematics as well as printed circuit boards.
It is diffused in particular in the italian Usenet community from late 1990s.

It can be freely downloaded (a Windows\index{Windows} version nationalized in Italian language) from Lorenzo Lutti's page\index{Lorenzo Lutti}:

\href{http://www.enetsystems.com/~lorenzo/fidocad.asp}{http://www.enetsystems.com/~lorenzo/fidocad.asp}

The output files provided by this software contains only text only codes and descritpions and it is very compact. Those details makes it very easy to include the drawings in text messages, such as those used in non binary Usenet groups\index{Usenet group}.

Unfortunately, FidoCad exists only in a Windows\index{Windows} version.
Who uses Linux can run it using WInE\index{WInE}, but for those using other platforms like me (I use MacOSX) have to find a different solution. I thus decided to give a small contribution to the Usenet community by writing FidoCadJ (with the final J, this time). This editor is written in pure Java\index{Java} and it is completely multi-platform. FidoCadJ allows showing and modifying a drawing using the FidoCad\index{FidoCad} file format.

Whoever used FidoCad\index{FidoCad} in the past should become acquainted to FidoCadJ very quickly, since a lot of commands and procedures are pretty similar in the two softwares.
Presently, to the best of my knowledge, FidoCadJ is almost completely compatible with the original FidoCad, apart a few details. Probably the library and macro\index{macro} handling can in particular be improved.

My goal has been to offer a minimalistic yet complete solution to drawing needs of small schematics as well as simple printed circuit boards (P.C.B.). I tried to respect as much as possible the original FidoCad philosopy, which was to use a simple tool which was completely open.
Of course, my middle term goal is to achieve 100\% compatibility with the original FidoCad\index{FidoCad} for Windows\index{Windows}.

Among the features offered by FidoCadJ and not present in the original FidoCad  there is the export possibilities offered. Since I am a LaTeX user, I made FidoCadJ export drawings in several vectorial formats, including encapsulated Postscript (also known as \textsc{EPS}\index{EPS}).
Another interesting format useful for electronic schematics is the export towards a CadSoft Eagle script\index{CadSoft Eagle}, available from version 0.21 of FidoCadJ. In this way, a schematics drawn with FidoCadJ can be exported to Eagle.

\section{History of this software}
I have long been interested in electronic circuits.
When I began following several dedicated italian Usenet newsgroups\index{newsgroups}, I noticed that a lot of schematics were provided using the FidoCad for Windows format\index{FidoCad}. This avoided ankwards ASCII drawings.
Since I do not use Windows since a few years ago, it was almost impossible for me to look at them and I wanted to try to do something to solve that problem.
\graffito{By the way, I think that it is better to work at a solution rather to claim that an operating system different from Windows lacks of software}.

The first thing I did has been to study the file format used by FidoCad\index{FidoCad} and write a Java\index{Java} applet called FidoReadJ\index{FidoReadJ}, able to parse the circuit and to show it in a web page. I started searching more or less everywhere (old posts, web pages) and I did a lot of reverse engineering from existing FidoCad files. I downloaded the FidoCad sources. They have been written by Lorenzo Lutti in C++ and are pretty neat.

I did this work more or less around March 2007. A few moths later, the applet was on line and it was being tested by part of the community gravitating around it.hobby.elettronica\index{it.hobby.elettronica} and it.hobby.fai-da-te\index{it.hobby.fai-da-te}.\footnote{FidoReadJ\index{FidoReadJ} is still available, even if not regularly updated, at the address:\\ \href{http://davbucci.chez-alice.fr/index.php?argument=elettronica/fidoreadj/fidoreadj.inc\&language=Francais}{http://davbucci.chez-alice.fr/index.php?argument=elettronica/fidoreadj/fidoreadj.inc}.}

Since I had an interpreter\index{interpreter} of the FidoCad format, it was interesting to continue the work in order to obtain a complete editor\graffito{In reality, I made a first attempt of writing a 2D vectorial drawing system arount 1993.}
The most part of the work has been done in several steps, between january and july 2008. FidoCadJ is not an adaptation or a porting de FidoCad for Windows, but it is a complete rewrite of the program.

The choice of using Java\index{Java} has been due to the fact that in these last years I changed a lot of operating systems. I do not want anymore to dedicate my efforts to something which is not completely portable. The effor of learning the Cocoa framework would have probably given a better result under MacOSX, but it would have made FidoCadJ completely non portable.
I am not a computer guy. The time I spend to program is time vasted for my electronics interests.
Anyway, a simple analysis of the FidoCadJ code source shows that I am not a very Java\index{Java} and object oriented programming\index{Object oriented programming} purist. Several solutions are more practical than elegant.

What it matters, much more than the choice of a particular language, is the end user impressions while using the program. For that reason, I am always hearing your suggestions about FidoCadJ, in order to understand how to develop this project.
To resume, I do not believe Java\index{Java} a perfect language. I believe it is criticized by many people because there are a lot of programs which are not very well integrated in the user desktop.

% La parte non tradotta comincia qui...

 Senza avere pretese di perfezione e restando 
...
\textsc{jpg}\index{JPG}
&
Diffusissimo formato bitmap. Per il fatto che la compressione utilizzata è lossy\index{compressione lossy}, non si presta bene per l'esportazione di schemi come quelli di FidoCadJ. È comunque disponibile per via della sua diffusione.
\\

\textsc{png}\index{PNG}
&
Formato bitmap compresso, adatto all'esportazione di schemi e grafici. In mancanza di un formato vettoriale, questo è probabilmente il formato migliore per esportare un disegno FidoCadJ.
\\

\textsc{svg}\index{SVG}
&
Formato vettoriale standard del W3C. Alcuni browser Internet (come le versioni recenti di Safari\index{Safari}) permettono di visualizzarlo all'interno di una pagina web. Ottimo formato per grafici e schemi, permette di usare programmi come Inkscape\index{Inkscape} per ritoccare i disegni forniti da FidoCadJ. Attualmente, ci sono alcune limitazioni per quanto riguarda l'esportazione del testo ruotato o specchiato.
\\

\textsc{eps}\index{EPS}
&
Formato vettoriale Postscript\index{Postscript} incapsulato. Molto utile per chi utilizzi programmi di grafica professionale, oppure voglia includere delle figure in documenti \LaTeX\index{LaTeX@\LaTeX}. L'esportazione dei disegni dovrebbe essere completa. Questa è la tecnica utilizzata in questo manuale per ottenere la figura~\ref{fig_amplificateur}, a pagina~\pageref{fig_amplificateur} (passando per una conversione in \textsc{pdf}, dato che utilizzo \textsc{pdf}\LaTeX\index{pdfLaTeX@\textsc{pdf}\LaTeX}).
\\

\textsc{pgf}\index{PGF}
&
Formato vettoriale da utilizzare direttamente all'interno di un file \LaTeX\index{LaTeX@\LaTeX}, che utilizzi il pacchetto \textsl{pgf}, disponibile nell'archivio CTAN. Questa modalità di esportazione è stata pensata in particolare per l'esportazione di schemi e fornisce uno script facilmente interpretabile e modificabile a mano. Gli attributi del testo non vengono tradotti. Questo permette di introdurre codice \LaTeX\index{LaTeX@\LaTeX} direttamente all'interno del disegno ed è la tecnica adottata in questo manuale per ottenere la figura~\ref{fig_schema}, a pagina~\pageref{fig_schema}.
\\
\textsc{scr}\index{SCR}
&
FidoCadJ, a partire dalla versione 0.21, permette di esportare un disegno verso uno script interpretabile dal programma CadSoft\index{CadSoft} Eagle\index{Eagle}. Per utilizzare questa possibilità, bisogna installare nella directory \lstinline!lbr! dell'installazione corrente di Eagle la libreria \lstinline!FidoCadJLIB.lbr!, scaricabile dal sito di FidoCadJ. Attualmente, l'esportazione è possibile solo per gli schemi elettrici che adottano i simboli più comuni. Non sono supportate alcuni elementi di disegno come le piazzole e le piste, che non saranno introdotte nello script Eagle.
\\
\bottomrule
\end{tabular}
...
Un'altra possibilità interessante, sebbene a stretto rigore sia più una caratteristica di Java\index{Java} che di FidoCadJ, è la possibilità di modificare l'aspetto del programma (che in gergo Java si chiama look \& feel\index{look \& feel}). Questo si può fare per esempio se si vuole integrare maggiormente il programma all'interno di un ambiente Windows\index{Windows}, nel qual caso, la linea di comando da utilizzare per far partire il programma è la seguente:\graffito{Java a volte richiede di scrivere di comandi ben poco concisi!}
\begin{lstlisting}
...
Essa è identificata dal comando \lstinline!TY!\index{TY}, seguito da svariati parametri, atti a determinare l'orientamento del testo (testo ruotato\index{testo!ruotato} o specchiato\index{testo!specchiato}), nonché le dimensioni in $x$ e $y$ del font utilizzato. Data la quantità di informazioni da fornire, la stringa di comando è abbastanza articolata:
\begin{lstlisting} 
TY x1 y1 sy sx a s l f £testo da scrivere
\end{lstlisting}
il punto $(x_1, y_1)$ è dove la stringa ``testo da scrivere'' verrà posizionata.
Il valore di $s_y$ e $s_x$ indica la dimensione verticale ed orizzontale del testo in unità logiche\index{unità logica}. Ho scelto di fare in modo che FidoCadJ rispetti la dimensione verticale del testo a partire da quella orizzontale, e che deformi il font solo se strettamente necessario.
...
Il layer è rappresentato dal solito termine $l$, mentre $f$ indica il font da utilizzare, oppure è un asterisco\index{asterisco}, per indicare l'utilizzo del font standard Courier\index{Courier}, di cui un esempio è mostrato in figura~\ref{fig_courier_sample}. Qualora il nome del font avesse degli spazi, questi vengono rimpiazzati dal simbolo +\index{+}.
Nello stato attuale delle cose, l'unico font usato e supportato da FidoCadJ è il Courier\index{Courier}, font a spaziatura fissa e con grazie, che è comunque il font di default adottato anche da FidoCad\index{FidoCad}.

\begin{figure}
\centering
\includegraphics[width=0.4\textwidth]{courier_sample}
\caption{Un esempio del font Courier, utilizzato da FidoCadJ.}
...
La primitiva connessione elettrica\index{connessione} è semplicemente un circoletto pieno di dimensione costante e viene utilizzata per rappresentare una connessione in uno schema elettrico. È identificata dal comando \lstinline!SA!\index{SA} e richiede unicamente le coordinate ed il layer:
\begin{lstlisting} 
SA x1 y1 l
\end{lstlisting}
...
\label{FCJ_extension}
A partire dalla versione 0.21, FidoCadJ ha introdotto delle estensioni\index{estensione FidoCadJ} al formato FidoCad originale, che sono identificate dal comando \lstinline!FCJ!\index{FCJ}. Per il momento, la sola che è stata implementata è quella che permette di associare un nome ed un valore ad ogni macro. L'utilizzo è il seguente:
\begin{lstlisting} 
[FIDOCAD]
MC 40 30 0 0 080
FCJ
...
\end{lstlisting}
La presenza del comando \lstinline!FCJ! indica che la definizione della macro non è terminata, ma che i campi indicanti il nome ed il valore sono specificati dai due comandi \lstinline!TY! nelle linee che seguono immediatamente \lstinline!FCJ!. Attualmente, solo le coordinate dei due comandi \lstinline!TY! sono utilizzate.

Questo modo di procedere ha il vantaggio che se un file contenente un'estensione di questo tipo viene letto da FidoCad\index{FidoCad}, quest'ultimo ignorerà le linee contenenti  \lstinline!FCJ! (emettendo un messaggio di errore), ma il disegno ottenuto sarà identico a quello che appare su FidoCadJ.

FidoCadJ può essere configurato per non emettere il comando \lstinline!FCJ!\index{FCJ} durante il salvataggio di un file, oppure le operazioni di copia/incolla. In questo caso, si otterrà un file perfettamente compatibile con FidoCad\index{FidoCad}, a prezzo della perdita dell'associazione logica tra una macro, il suo nome ed il suo valore.
...
Nulla impedisce di richiamare una macro all'interno di un'altra macro. Quello che bisogna evitare, ovviamente, è la ricorsione\index{ricorsione}, ovvero di scrivere una macro che richiama sè stessa.
...
In questo manuale, si è visto come utilizzare FidoCadJ per disegnare uno schema elettrico oppure un semplice circuito stampato. A questo punto, il lettore dovrebbe possedere tutti gli elementi necessari per utilizzare profiquamente FidoCadJ per le proprie esigenze.
...
\myChapter{Estensioni specifiche per piattaforma}
\label{specifics}
\section{MacOSX}
Una delle critiche più diffuse alle prime versioni di FidoCadJ da parte degli utilizzatori Macintosh (come me, del resto) consiste nella cattiva integrazione del programma sotto MacOSX. A partire dalla versione 0.21.1, FidoCadJ fa degli sforzi specifici per integrarsi all'interno dell'aspetto e del funzionamento delle applicazioni native.
...
\myChapter{Installazione di FidoCadJ}
\label{installazione}

\section{Come scaricare ed eseguire FidoCadJ con MacOSX}
...
\section{Come scaricare ed eseguire FidoCadJ su un sistema Linux}
\label{installazione_linux}
\textsl{di Roby IZ1CYN}\\
...
Nel paragrafo \ref{inst_testo} descriveremo come installare il programma adoperando esclusivamente comandi da terminare. Nel paragrafo \ref{inst_grafica}, ci baseremo invece sull'interazione tramite un sistema grafico.
...
\item {Selezioniamo la scheda ``Apri con'' e scegliamo ``OpenJDK Java 6 Runtime'' oppure ``Sun Java 6 Runtime'', come visibile nella figura~\ref{fig_java6}.}
