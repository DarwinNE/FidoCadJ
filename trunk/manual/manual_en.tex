...
\title{\Huge\color{webbrown} FidoCadJ 0.22 \\ the user manual} 
...
This software has been very carefully tested under Linux thanks to
...
for their tests under MacOSX.

I would like to thank ``F. Bertolazzi'' who has given very useful
advice about the usability of this software. He also assembled the
CadSoft Eagle\index{Eagle} compatibility library, useful to export
FidoCadJ drawings under Eagle. Many thanks to ``Celsius'', who tested
...
aspect on the Apple Macintosh, from the 0.21.1 version.

This manual has been translated to English by ``Pasu''. I would like to thank 
him for his work.
...
\myChapter{Introduction} \pagenumbering{arabic} In this chapter,
...
FidoCad\index{FidoCad} (without the J at the end) is a vectorial
...
descritpion. This feature makes it very easy to include drawings in
...
with the original FidoCad, apart from a few details. In particular
the library and macro\index{macro} handling could probably be improved.

My goal has been to offer a minimalist yet complete solution to
drawing needs of small schematics as well as simple printed circuit
boards (P.C.B.). I tried to respect as much as possible the original
FidoCad philosophy, which was to use a simple tool which was completely
open. Of course, my middle term goal is to achieve 100\% compatibility
with the original FidoCad\index{FidoCad} for Windows\index{Windows}.
...
formats, including Encapsulated PostScript (\textsc{EPS}\index{EPS}).
...
this problem. \graffito{By the way, I think that it is better to
...
\footnote{FidoReadJ\index{FidoReadJ} is still available, even if not regularly
...
editor\graffito{To be honest, I made a first attempt at writing
...
have probably given a better result under MacOSX, but it would have
...
\myChapter{Drawing with FidoCadJ}

The use of FidoCadJ should be quite intuitive for those who have ever
used a vectorial drawing application\index{vectorial drawing}. A
screenshot of the program running under MacOSX\index{MacOSX} is shown
in Fig.~\ref{fig_fidocadj};\graffito{Expert Mac-users will notice
that all the menus are at their own place!} A few details may be
different when running under other operating systems (for example
Fig.~\ref{fig_metal} shows the result with Look and Feel Metal\index{Metal}
on Sun\index{Sun}), but the philosophy remains the same. We will
...
\caption{A typical FidoCadJ session running under MacOSX Tiger\index{MacOSX}.
...
position until another function from the toolbar will be selected.\graffito{This
...
%
\begin{table}
\centering \begin{tabular}{clp{0.7\textwidth}}
...
(\keyevidence{Command} only under MacOSX) for multiple selections
...
\textsc{Rectangole}\index{rectangle}  & Draws a rectangle filled or empty.\tabularnewline
...
use is called {}``NPN senza schermo'' and is included in the {}``Diodi
e transistor'' category of the {}``Libreria standard''. By clicking
on the element's name to select the desired macro\index{macro}, it
will then be possible to place it anywhere in the drawing, by clicking
...
its mirrored version. We will thus obtain a result similar to the
...
will be able to make a few electrical connections, until we will realise
...
a resistor\index{resistor} (Libreria standard/Componenti discreti/Resistore)
and the label for the positive power supply (Libreria standard/Simbologia
di base/Terminale +). We will need to rotate the latter in order to
...
there is a macro called {}``Freccia'', contained in {}``Libreria
standard/Simbologia di base''. To place the text, we can press the
...
and ``half units\textquotedbl{} are not allowed, meaning that the
...
if this is a complex one, is not an easy task. \graffito{The reader
will find their way through: a few scribbles made with paper and pencil
(and a lot of erasers) will result in time saved by obtaining a clear
idea to be developed with the use of the computer.} The autoplacer\index{autoplacer}
...
(from the library {}``PCB footprints/Semiconduttori 3 terminali/TO92''),
the resistors ({}``PCB footprints/Resistori/Resistore 1/4 W 0,4 i''),
and the electrolytic capacitors ({}``PCB footprints/Condensatori
elettrolitici/Vert. diam. 5 passo 2,5''). In order to delimitate
...
\footnote{FidoCadJ is currently available in Italian\index{Italin}, Italian
with Swiss variants, French\index{French} and English\index{English}.
...
primitive\index{PCB line}. I chose a track thickness of 10 unità
...
designed for a polyester capacitor. \graffito{Pay attention to the
...
the soldering.} It will probably have the right dimensions. Let's
...
place a ceramic capacitor in parallel with the power supply. Let us
...
above (pay attention to the long lines!):
\begin{lstlisting} 
[FIDOCAD]
...
list of graphic file formats currently available. \graffito{A vectorial
...
This can be done for example to better integrate the program within
a Windows environment\index{Windows}, in which case the command line
to be used would be the following:\graffito{Java sometimes requires
the writing of not so brief commands!} 
\begin{lstlisting} 
java -Dswing.defaultlaf=com.sun.java.swing.plaf.windows.WindowsLookAndFeel -jar fidocadj.jar 
\end{lstlisting} 
For license reasons, the Windows\index{Windows}
look and feel is available only on Microsoft\index{Microsoft} systems.
...
look \& feel, shown in Figure~\ref{fig_fidocadj_motif}: 
\begin{lstlisting}
...
\caption{The appearance of the program under MacOSX\index{MacOSX}, using the Motif look \& feel.}
...
The program allows us to specify a directory in which all the library\index{library}
files are placed (extension \lstinline!.fcl!). This can be done through
the menu {}``View/Options''. If a file named \lstinline!FCDstdlib.fcl!
is present, its content will supersede the standard library\index{library!standard library}
directly available in the application. Analogously, if a file named
\lstinline!PCB.fcl! is present, its content will replace the PCB
library%
\footnote{Pay attention to the use of capital letters, in particular if your
...
\myChapter{Drawing format, macros and FidoCad libraries} \label{chap_formato}
...
schematic to make better use of the size of the page otherwise it
will print something the size of a postage stamp.
...
La line primitive\index{line}\graffito{Mathematicians would probably
...
while $l$ is the layer, characterized by a whole number between 0
and 15.
...
the use of the standard Courier font\index{Courier}, of which an
example is shown in Figure~\ref{fig_courier_sample}. If the font
name contains spaces these must be replaced with the symbol +\index{+}.
At the moment the only font supported by FidoCadJ is the Courier\index{Courier},
a constant width font which is also the default font adopted by FidoCad\index{FidoCad}.

%
\begin{figure}
\centering \includegraphics[width=0.4\textwidth]{courier_sample} 

\caption{An example of Courier font, used in FidoCadJ.}
...
\toprule Bit  & Peso  & Funzione \tabularnewline
...
\subsubsection{Empty or filled polyline}
...
\label{FCJ_extension} Starting from version 0.21, FidoCadJ introduced
...
will be identical to the one produced by FidoCadJ.

FidoCadJ can be configured in order to avoid the command \lstinline!FCJ!\index{FCJ}
when saving a file, or during cut/paste operations. In this case,
we will obtain an output perfectly compatible with FidoCad\index{FidoCad},
at the price of losing the logical association between a macro, its
name and its value.
...
\myChapter{Conclusion} We have seen in this manual how to use FidoCadJ
...
FidoCadJ should not be thought of as a tool meant uniquely for the
...
The advantages of a free program is that it is at the disposition
of the community. For this reason the feedback from the users is very
important (at least to understand if the project is worth to be developed
further, and in which direction). To contact me, you can write an email
(\href{mailto:davbucci@tiscali.it}{davbucci@tiscali.it}, but do
not send me any attachment!).

\appendix

\myChapter{Platform-specific extensions} \label{specifics} 
...
integration of the program under MacOSX. Starting from version 0.21.1,
...
\myChapter{FidoCadJ installation} \label{installazione}


\section{How to download and execute FidoCadJ under MacOSX}

FidoCadJ can run under a MacOSX version equal to or greater than 10.3.9
(Panther). The reason is simple: FidoCadJ requires at least version
1.4 of Java, which Apple supplies with their operating system.

Although it is possible to use the file \lstinline!fidocad.jar! like
with other operating systems, it is preferable to use the specific
application under MacOSX. Everything is dealt with as in native applications:
it is sufficient to download the disk image containing the program
from the url: \href{http://davbucci.chez-alice.fr/elettronica/fidocadj/FidoCadJ.dmg}{http://davbucci.chez-alice.fr/elettronica/fidocadj/FidoCadJ.dmg},
open it and move \lstinline!FidoCadJ.app! into the folder \lstinline!Applications!
from which it will then be available.

To uninstall FidoCadJ, it is sufficient to delete the file \lstinline!FidoCadJ.app!
from the \lstinline!Applications! folder by moving it into the trash.
...
with a graphical environment will be used instead.
...
--23:51:22--  http://davbucci.chez-alice.fr/elettronica/fidocadj/fidocadj.jar
           => `fidocadj.jar'
Resolution of davbucci.chez-alice.fr is being done... 212.27.63.127
Connection to davbucci.chez-alice.fr|212.27.63.127:80... connected.
HTTP request sent, waiting for answer... 200 OK
Length: 276,694 (270K) [application/x-java-archive]

100%[====================================>] 276,694       71.75K/s    ETA 00:00

23:51:26 (71.59 KB/s) - "fidocadj.jar" saved          [276694/276694]
\end{lstlisting}
\lstset{
	basicstyle=\small\ttfamily}
...
\item {We can then launch our File Manager (Nautilus, Konqueror\dots) as a root (if we do not have a specific command in the menu, we just need to launch it from the console by using \lstinline!sudo nautilus!). We can create the directory \lstinline!/usr/bin/fidocadj! and move there the file we just downloaded (it will be somewhere in \lstinline!/home/<user>/!)}

\begin{figure}
\centering
\includegraphics[width=.8\textwidth]{permessi}
\caption{The setting of file rights, under Ubuntu\index{Ubuntu} 8.04.}
\label{fig_permessi}
\end{figure}
...
\caption{Set the execution with the Java virtual machine, under Ubuntu\index{Ubuntu} 8.04.}
\label{fig_java6}
\end{figure}
...
\mbox{ }
\vfill
This manual has been written using \textsc{pdf}\LaTeX\ under MacOSX, with the \href{http://www.ctan.org/tex-archive/macros/latex/contrib/classicthesis/}{classicthesis} class.
