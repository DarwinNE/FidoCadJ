...
\title{\Huge\color{webbrown} FidoCadJ 0.23.4 \\ le manuel de l'utilisateur} 
...
Du temps que je dédie à FidoCadJ, seulement une moitié est dédiée à travailler sur le code source. Le reste est occupé par la formulation de réponses aux utilisateurs et le soin de la documentation.
...
&
Sélection d'un ou plusieurs éléments graphiques. Appuyer sur \keyevidence{Control} (ou \keyevidence{Command} sous MacOSX) pour des sélections multiples, ou dé-sélectionner.
Faire clic et glisser la souris en maintenant appuyé le bouton gauche, pour sélectionner plusieurs éléments dans un rectangle.
...
\textsc{Pastille}\index{pastille c. i.}
&
...
On peut remarquer tout de suite que le transistor bipolaire sur la gauche n'est pas orienté comme il conviendrait. Il suffit de cliquer sur le bouton \og  Sélectionner\fg , dans la barre des outils, sélectionner le transistor avec la souris et appuyer sur la touche \keyevidence{S}. Une alternative consiste à appuyer sur la touche  \keyevidence{S} lorsque le symbole du transistor a été pris dans les bibliothèques, avant de l'insérer dans le dessin. Les éléments sélectionnés apparaîtront marqués en vert. Le résultat que l'on va obtenir devrait être similaire à celui montré en figure~\ref{fig_fidocadj_fase2}.
...
Tout cela est fait au démarrage de FidoCadJ, ou quand l'utilisateur change l'affectation du répertoire. On peut aussi forcer une remise à jour manuelle avec l'option \og Mettre à jour les bibliothèques \fg\ du menu \og Circuit \fg .

À partir de la version 0.23, grâce à l'accord de Roby IZ1CYN, j'ai pu inclure la bibliothèque IHRaM 3.0 directement à l'intérieur de l'archive contenant FidoCadJ. Cela parce que parmi les nombreuses bibliothèques mises au point par les utilisateurs elle m'a apparue comme la plus complète et rationnelle.
...
Dans ce chapitre, nous allons décrire en détail le format utilisé par FidoCad et donc aussi par FidoCadJ pour mémoriser les dessins. Il s'agit d'un très simple format de texte qui a l'avantage d'être compact et efficace. Comme ce format n'a jamais été décrit de façon complète, je propose ici un résumé de tout ce que j'ai appris pendant l'étude que j'ai faite. Avant de commencer, je rappelle que depuis la version 0.23.4, FidoCadJ utilise comme encodage\index{encodage} le UTF-8\index{UTF-8}, dans toutes les situations.
